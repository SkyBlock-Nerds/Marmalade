package net.hypixel.nerdbot.marmalade.storage.database.model.user.stats;
...
public class LastActivity {
    ...
    public void addChannelHistory(String channelId, String channelName, long lastMessageTimestamp) {
        addChannelHistory(channelId, channelName, 1, lastMessageTimestamp);
    }

    public void addChannelHistory(String channelId, String channelName, int amount, long timestamp) {
        channelActivityHistory.stream()
            .filter(entry -> entry.getChannelId().equals(channelId) || entry.getLastKnownDisplayName().equalsIgnoreCase(channelName))
            .findFirst()
     ...
     */
    public void addRoleRestrictedChannelActivity(String groupIdentifier, String channelId, String channelName, int amount, long timestamp) {
            ...
            .ifPresentOrElse(entry -> {
                ...
                if (entry.getLastKnownDisplayName() == null || !entry.getLastKnownDisplayName().equalsIgnoreCase(channelName)) {
                    log.info("Updating role-restricted channel activity entry for group {} channel {} (ID: {}) with new display name: {}",
                        groupIdentifier, entry.getLastKnownDisplayName(), channelId, channelName);
                    entry.setLastKnownDisplayName(channelName);
                }

                log.info("Updated role-restricted channel activity entry for group {} channel {} (ID: {}): {} messages",
                    groupIdentifier, channelName, channelId, entry.getMessageCount());
            }, () -> {
                log.info("Adding new role-restricted channel activity entry for group {} channel {} (ID: {})",
                    groupIdentifier, channelName, channelId);
                groupHistory.add(new ChannelActivityEntry(channelId, channelName, amount, timestamp,
